...
```
    1. A user sends a request to access the web content on a browser using the website address (link).

    2. The server serves up the static files (CSS and HTML) to the client’s browser on the client’s first request for the website.

    3. The client browser will download the HTML content first and then JavaScript. These HTML files link the JavaScript. This loading process happens as the user sees the loading symbols defined by the web developer. The site is not yet visible to the user.

    4. After the browser downloads the JavaScript, content is dynamically generated on the client’s browser.

    5. The web content becomes visible as the client navigates and interacts with the website.
```
...
```
```
The JavaScript frameworks that supports client-side rendering development style includes React, Angular, and Vue.js.
...
#### _`1. SOAP (Simple Object Access Protocol) - `_
```
is an XML-based messaging protocol for exchanging information among computers. SOAP’s built-in WS-Security standard uses XML Encryption, XML Signature, and SAML tokens to deal with transactional messaging security considerations. SOAP also supports OASIS and W3C recommendations.
...
```
#### _`2. REST (Representational State Transfer) - `_
```
It uses HTTP to obtain data and perform operations on remote computer systems. It supports SSL authentication and HTTPS to achieve secure communication.
...
#### _`2. API injections (XSS and SQLi) - `_
```
...
#### _`3. Distributed denial of service (DDoS) - `_
```
...
### _`1. Authentication - `_
```
Determining the identity of an end user. In a REST API, basic authentication can be implemented using the TLS protocol, but OAuth 2 and OpenID Connect are more secure alternatives.
```
### _`2. Authorization - `_
```
...
> Server performance - 
```
– There is a correlation between server performance metrics—including server speed, hardware used (e.g., HDD/SDD drives) and available RAM—and your site latency.
```
> Round-trips - 
```
– A round-trip is the journey taken by an object request (e.g., HTML files, stylesheets and script files) to your webserver and back to the user. Round-trip time (RTT) is primarily affected by the distance between webserver and user, as well as the number of intermediate points through which a connection travels.
```
